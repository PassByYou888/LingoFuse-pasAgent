...
如果你只想跑通 MCP 闭环，可先按 `MCP_SERVER_DOUBAO_GUIDE.md` 操作，模型下载部分参考本文档第三章。
...
详细参数说明见 `src/llm-service/LingoFuse_LLM_Service_CLI_guide.md`。

### 4.5 替代方案：用 llm_proxy.exe 转发
...
详细用法见 `src/llm-service/LingoFuse_LLM_Proxy_CLI_Guide.md`。
...
| `LingoFuse_LLM_Proxy_Compatibility_Guide.md` | `src/llm-service/` | 支持的 129+ 后端清单 |
| `LingoFuse_LLM_Pitfalls_For_AI.md` | `src/llm-service/` | 踩坑大全 |
| `LingoFuse_LLM_Service_Work_Summary.md` | `src/llm-service/` | 版本演进与架构决策 |
| `llama_cpp_python_guide.md` | `src/llm-service/` | `llama-cpp-python` 安装与使用 |

---

**文档版本**：V2.0（高对比配色，拆分图表）  
