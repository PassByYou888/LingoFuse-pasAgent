# LingoFuse LLM Pitfalls For AI（v2.0）

> 文件路径：`LingoFuse_LLM_Pitfalls_For_AI.md`
> 目标读者：**AI 助手**
> 用途：接手本项目时，快速避坑
> 覆盖范围：服务端 / 代理层 / 客户端 / 协议 / 跨语言 / GUI
> 版本：v2.0（2026-09-13）—— 合并 llm_proxy.py 迭代与 new_session 实现
...
**版本变化（v2.0 新增）**：
- 新增 `llm_proxy` 专项章节（第三章）
- 新增 `P0-4` / `P0-5`：SSE 传输层缓冲与进程生命周期
- 新增 `P1-8` / `P1-9`：set_system_message 静默失效、会话过滤
- 新增 `P2-3`：Windows 控制台 emoji
- 新增 `P4-5` / `P4-6`：new_session 实现与温和失败处理
...
flowchart TB
    ...
    style ROOT fill:#E74C3C,stroke:#922B21,stroke-width:5px,color:#FFFFFF
    style A fill:#3498DB,stroke:#1F618D,stroke-width:3px,color:#FFFFFF
    style B fill:#2ECC71,stroke:#1E8449,stroke-width:3px,color:#FFFFFF
    style P fill:#8E44AD,stroke:#5B2C6F,stroke-width:3px,color:#FFFFFF
    ...
    style E fill:#E67E22,stroke:#9C4A0C,stroke-width:3px,color:#FFFFFF
```
...
for line in resp.iter_lines(decode_unicode=True):        # 512 字节缓冲
    ...

for chunk in resp.iter_content(chunk_size=None):         # 读直到 EOF
    ...

for chunk in resp.iter_content(chunk_size=1):            # urllib3 仍预读
    ...

headers["Accept-Encoding"] = "identity"                  # 单改这一个无效
```
...
**为什么 http.client 是唯一可靠方案**：
- `HTTPResponse` 直接包 socket 上的 `BufferedReader`
- `for line in resp:` 走 `readline()`，socket 有新行立即返回
- 没有任何中间层加缓冲
...
**错误认知**：
```
"LM Studio 每秒输出 30 token，但客户端延迟几秒"
→ 实际上：LM Studio 秒级输出的 30 token 全是 reasoning_content
→ 代理正确转发到了后端，但按策略丢弃了
```
...
**根因**：
Free Pascal 在重载决议时，`var` 和 `out` 编码为**相同的引用传递类别**。这两个函数在编译器眼里是一个：
...
**根因**：
`Connect` 有多条失败路径，每条都独立处理：
```pascal
FApp := LF.TAppHandle.Create(...);
...
```
TZ_JsonString (Unicode) → string (AnsiString?) → UTF8Encode → 服务端
                              ↑
                         这里丢字
```
...
begin
  LLM := TLLMClient.Create(llm_APP_Edit.Text, ...);  // UI 读取
end;
```
...
`LF_Shutdown` 会强杀 LingoFuse，但 `LLM` 对象没被 `Disconnect`，且正在执行的回调线程被中断。
...
begin
  ...
  TCompute.SyncM(Do_Thread_Connect_Done);  // ✅ 只在成功时恢复
end;
```
...
flowchart TB
    ...
    style PASS fill:#2ECC71,stroke:#1E8449,stroke-width:5px,color:#FFFFFF
    ...
    style F3 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
    style F4 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
    style F5 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
    style F6 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
    style F7 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
    style F8 fill:#E74C3C,stroke:#922B21,stroke-width:3px,color:#FFFFFF
```
...
```mermaid
flowchart LR
    LFC["LingoFuse client<br/>(LF binary RPC)"] -->|"Call + Notify"| Proxy["llm_proxy.py<br/>(翻译器)"]
    ...
    style Proxy fill:#8E44AD,stroke:#5B2C6F,stroke-width:5px,color:#FFFFFF
    style LFC fill:#3498DB,stroke:#1F618D,stroke-width:3px,color:#FFFFFF
    style Backend fill:#2ECC71,stroke:#1E8449,stroke-width:3px,color:#FFFFFF
```
...
flowchart LR
    ...
    R4 --> R5["5. llm_client.pas<br/>看跨语言对齐"]
    R5 --> R6["6. llm_tool_frm.pas<br/>看 GUI 集成"]

    style R1 fill:#E74C3C,stroke:#922B21,stroke-width:5px,color:#FFFFFF
    style R2 fill:#3498DB,stroke:#1F618D,stroke-width:3px,color:#FFFFFF
    style R3 fill:#8E44AD,stroke:#5B2C6F,stroke-width:3px,color:#FFFFFF
    style R4 fill:#2ECC71,stroke:#1E8449,stroke-width:3px,color:#FFFFFF
    style R5 fill:#9B59B6,stroke:#6C3483,stroke-width:3px,color:#FFFFFF
    style R6 fill:#F5A623,stroke:#B7791F,stroke-width:3px,color:#FFFFFF
```
...
5. `llm_client.pas` 的 `OnLLMStream`（协议入口）
6. `llm_client.pas` 的 `CleanupPartialConnect`（资源安全）
7. `llm_tool_frm.pas` 的 `new_session_ButtonClick`（system prompt 生效入口）
...
**文档完成**
*本踩坑总结面向 AI，通过症状-根因-正确做法的结构化形式，让后续 AI 助手能快速定位和避免已知陷阱。v2.0 合并了 llm_proxy.py 从 v1.0 到 v1.8 的迭代经验、llm_test.py 的 emoji 支持、以及 new_session 的实现要点。*