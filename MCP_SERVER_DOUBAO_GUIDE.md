...
flowchart LR
    ...
    B --> C["3️⃣ MCP 网关<br/>mcp_server.exe"]
    C --> D["4️⃣ LLM 服务<br/>llm_service.exe"]
...
| `mcp_server.exe` | MCP 网关，把工具翻译成聊天软件能听懂的语言 | ❌ 不行 |
| `mcp_proxy.exe` | 调试侦探，用来查哪里出错了 | ⚠️ 可选，但推荐有 |
| `llm_service.exe` | AI 的大脑，本地跑大模型 | ❌ 不行（除非用 `llm_proxy.exe`） |
| `llm_proxy.exe` | 转发到 LM Studio / Ollama / 云 API 的代理 | ⚠️ `llm_service.exe` 的替代方案 |
| `llm_test.exe` | 命令行测试客户端，用来验证 LLM 服务是否正常 | ⚠️ 可选 |
| `HealthCheck.exe` | 健康检查工具，快速验证环境 | ⚠️ 可选 |
| `NVIDIA-Nemotron-3.5-Lightning-30B-A3B-UD-IQ4_NL.gguf` | AI 的“知识库”模型文件（约 20 GB） | ❌ 不行（除非用 `llm_proxy.exe`） |
...
## 🔍 第零步（可选）：用 HealthCheck.exe 快速验证环境
...
### 替代方案：用 `llm_proxy.exe` 转发到 LM Studio
...
## 📄 第五步：生成聊天软件的配置文件（mcp_server.exe --generate-configs）
...
### Q2：聊天软件看不到工具
**A**：首先确认信标、注册员、LLM 服务、MCP 网关四个窗口都开着；其次检查 `mcp_server` 窗口有无错误；最后确认配置文件中的路径是**绝对路径**（比如 `C:\Temp\temp2\mcp_server.exe`）。如果不确定，**拍配置文件内容发给豆包**。
...
4. 开第三个 CMD，运行 `llm_service.exe` —— 保持开着（或用 `llm_proxy.exe` 替代）。
5. 开第四个 CMD，运行 `mcp_server.exe --generate-configs` —— 会退出，不用管。
6. 开第五个 CMD，运行 `mcp_server.exe` —— 保持开着。
